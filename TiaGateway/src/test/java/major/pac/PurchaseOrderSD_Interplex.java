...
public class PurchaseOrderSD_Interplex extends BaseUtility {
	...
	@Given("Launch Tia application Dashboard On the google chrome Browser {string} {string} {string}")
	...
	@Then("Validate Purchase Order Label, MenuButton, Account name with Logo and Notification Bell Icon are available on Purchase Order list page.")
	...
	@Then("Validate Status Filter on Purchase Order list page.")
	...
	@Then("Validate PartnerOrg Filter on Purchase Order list page.")
	...
	@Then("Validate Region Filter on Purchase Order list page.")
	...
	public void validate_the_select_action_on_purchase_order_list_page()
		...
		try {
			if (result) {
				...
				sa.assertEquals(text, "No data found for the selected date range. Please select proper date range.",
						"Generate Po Report Toast Message not shown");
				...
				sa.assertEquals(textt, "Please select purchase order to create PDF",
						"Please select purchase order to create PDF toast message is not shown");
				...
				sa.assertEquals(texttt, "Please select purchase order to create CSV",
						"Please select purchase order to create CSV toast message is not shown");
			...
			if (result1) {
				...
				WebElement toastmessage = driver.findElement(By.cssSelector("#feedbackMessageContainer > div > div"));
				String text = toastmessage.getText();
				sa.assertEquals(text, "File Downloading..",
						"For Generate PO report: File Downloading Toast messeage is not shown");
				String text2 = options.get(2).getText();
				...
				sa.assertEquals(textt, "File Downloading..",
						"For Download PDF: File Downloading Toast messeage is not shown");
				String text3 = options.get(3).getText();
	...
	@Then("Validate Shows Entries on Purchase Order list page.")
	public void validate_shows_entries_on_purchase_order_list_page() throws InterruptedException, IOException {
		...
		if (rowcount >= 10) {
			...
			System.out.println(equals+"10entries");
		...
		if (rowcount>=100) {
			...
			boolean equals4 = webtablerowcount4.equals(66);
	...
	@Then("Interplex Purchase order listing page Failures")
	...
	@Given("Launch Tia application On the chrome Browser {string} {string} {string}")
	...
	@Then("Validate Ship To Label and Logo, Bill To Label and Logo, and PO status are available on the Purchase order details page.")
	...
	@Then("Validate Plus button on the purchase order details page.")
	...
	@Then("Validate Search module on the purchase order details page.")
	public void validate_search_module_on_the_purchase_order_details_page() throws InterruptedException, AWTException {
		...
		Actions ac = new Actions(driver);
		String dat = "100";
		//po.getTextboxsrch().sendKeys(dat);

		ac.moveToElement(po.getTextboxsrch()).click().perform();
		ac.keyDown(Keys.NUMPAD2).perform();
		ac.keyUp(Keys.NUMPAD2).perform();
		...
		for (int i = 1; i < c; i++) {
			...
			System.out.println(webTable);
			boolean contains = webTable.contains("200");
			if (contains == false) {
				Result = false;
			}
	...
	@Then("Validate product details table on the purchase order details page.")
	public void validate_product_details_table_on_the_purchase_order_details_page()
		...
		List<Integer> procac = new ArrayList<>();
		for (int i = 1; i <= webtablerowcount; i++) {
			String webtableheadinnerText = webTableinnerText(i, 1, po.getProducttable());
			procac.add(Integer.parseInt(webtableheadinnerText));
		}
		Collections.sort(procac);
		List<Integer> procdc = new LinkedList<>();
		...
		List<Integer> prochigh = new LinkedList<>();
		for (int i = 1; i <= prodcrowcount; i++) {
			String webtableheadinnerText = webTableinnerText(i, 1, po.getProducttable());
			prochigh.add(Integer.parseInt(webtableheadinnerText));
		}
		...
		List<Integer> proclow = new LinkedList<>();
		for (int i = 1; i <= proacrowcount; i++) {
			String webtableheadinnerText = webTableinnerText(i, 1, po.getProducttable());
			proclow.add(Integer.parseInt(webtableheadinnerText));
		}
	...
	@Then("Interplex purchase order details page Failures")
