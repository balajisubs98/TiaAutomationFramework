...
public class ShipmentListSD_Rose_Macdonald extends BaseUtility {
	...
	@Given("Rose&Macdonald Launch Tia application Dashboard via google chrome Browser {string} {string} {string}")
	...
	public void validate_after_clicks_shipment_list_option_it_navigates_to_packing_and_shipments_list_page()
		...
		js.executeScript("arguments[0].click()", sp.getProcuretopay());
		takeScreenshot(screenshotpath, "OtoC");
		js.executeScript("arguments[0].click()", sp.getReceiveShipmentsAR());
		takeScreenshot(screenshotpath, "SendShipments_AR");
		js.executeScript("arguments[0].click()", sp.getShipmentsListAR());
		fwait(sp.getPackingAndShipmentsLabel());
		String title = driver.getTitle();
		takeScreenshot(screenshotpath, "Send_Shipment_Listing_Page_AR");
	...
	@Then("Rose&Macdonald Validate Purchase Order Label, MenuButton, Account name with Logo and Notification Bell Icon are available on Shipments list page.")
	...
	public void validate_from_date_filter_and_to_date_filter_on_shipments_list_page()
		...
		Thread.sleep(5000);
		takeScreenshot(screenshotpath, "Send_Shipments_Date_Filter");
	...
	@Then("Rose&Macdonald Validate Status Filter on Packing and Shipments list page.")
	public void validate_status_filter_on_packing_and_shipments_list_page()
		...
		for (int i = 0; i < li.size(); i++) {
			...
			sp.getStatusFiltersearch().sendKeys(li.get(i));
			Robot r = new Robot();
			r.keyPress(KeyEvent.VK_ENTER);
			r.keyRelease(KeyEvent.VK_ENTER);
			Thread.sleep(5000);
			takeScreenshot(screenshotpath, "Send_Shipments_Status_Filter");
			Integer webtablerowcount = webtablerowcount(po.getPotable());
	...
	public void validate_receiver_filter_on_packing_and_shipments_list_page()
		...
		for (int i = 0; i < li.size(); i++) {
			...
			sp.getRecvFiltersearch().sendKeys(li.get(i));
			Robot r = new Robot();
			r.keyPress(KeyEvent.VK_ENTER);
			r.keyRelease(KeyEvent.VK_ENTER);
			Thread.sleep(5000);
			takeScreenshot(screenshotpath, "Send_Shipments_Receiver_Filter");
			Integer webtablerowcount = webtablerowcount(po.getPotable());
			System.out.println("rowCount" + webtablerowcount);
			if (webtablerowcount != 0) {
				for (int j = 2; j < webtablerowcount; j++) {
	...
	@Then("Rose&Macdonald Validate PartnerOrg Filter on Packing and Shipments list page.")
	public void validate_partner_org_filter_on_packing_and_shipments_list_page()
		...
		for (int i = 0; i < li.size(); i++) {
			...
			sp.getPartorgFiltersearch().sendKeys(li.get(i));
			Robot r = new Robot();
			r.keyPress(KeyEvent.VK_ENTER);
			r.keyRelease(KeyEvent.VK_ENTER);
			Thread.sleep(5000);
	...
	public void validate_partner_filter_on_packing_and_shipments_list_page()
		...
		for (int i = 0; i < li.size(); i++) {
			...
			sp.getPartnerFiltersearch().sendKeys(li.get(i));
			Robot r = new Robot();
			r.keyPress(KeyEvent.VK_ENTER);
			r.keyRelease(KeyEvent.VK_ENTER);
			Thread.sleep(5000);
			takeScreenshot(screenshotpath, "Send_Shipments_Partner_Filter");
			Integer webtablerowcount = webtablerowcount(po.getPotable());
			System.out.println("rowCount" + webtablerowcount);
			if (webtablerowcount != 0) {
				for (int j = 2; j < webtablerowcount; j++) {
	...
	@Then("Rose&Macdonald Validate Region Filter on Packing and Shipments list page.")
	public void validate_region_filter_on_packing_and_shipments_list_page()
			throws AWTException, InterruptedException, IOException {
		po = new PurchaseOrderPOM();
		sp = new ShipmentListPOM();
		JavascriptExecutor js = (JavascriptExecutor) driver;
		...
		for (int i = 0; i < li.size(); i++) {
			...
			sp.getRegionFiltersearch().sendKeys(li.get(i));
			Robot r = new Robot();
			r.keyPress(KeyEvent.VK_ENTER);
			r.keyRelease(KeyEvent.VK_ENTER);
			Thread.sleep(5000);
			takeScreenshot(screenshotpath, "Send_Shipments_region_Filter");
			Integer webtablerowcount = webtablerowcount(po.getPotable());
			System.out.println("rowCount" + webtablerowcount);
			if (webtablerowcount != 0) {
				for (int j = 2; j < webtablerowcount; j++) {
					...
					if (equals == true) {
						...
						sa.assertEquals(true, webTable.contains(li.get(i)), "region Filter");
	...
	@Then("Rose&Macdonald Validate Shows Entries on Packing and Shipments list page.")
	...
	@Given("Rose&Macdonald Launch the Tia application on google chrome Browser {string} {string} {string}")
	public void interplex_launch_the_tia_application_on_google_chrome_browser(String string, String Username, String Password) {
		launchChromeBroswer("https://tia-qa.tangentia.com/TIA/LoginScreen");
		toMaximize();
		po = new PurchaseOrderPOM();
		fwait(po.getUsernmeTextbox());
	...
	public void interplex_validate_order_to_cash_receive_orders_order_list_select_a_po_from_list_click_button_should_be_reach_shipments_details_page() throws IOException, InterruptedException {
		po = new PurchaseOrderPOM();
		sp = new ShipmentListPOM();
		po.getMenuBtn().click();
		JavascriptExecutor js = (JavascriptExecutor) driver;
	...
	@When("Rose&Macdonald Validate PO or Delfore number on Shipments Details Page.")
	...
	public void interplex_validate_barcode_number_asn_date_asn_status_asn_date_created_date_shipment_id_shipment_date_delivery_date_transport_type_autopack_ship_from_ship_to_tracking_no_and_awb_no_label_are_available_on_shipments_details_page() throws InterruptedException {
	  sp = new ShipmentListPOM();
	  String text = sp.getpOordel().getText();
	  Thread.sleep(4000);
		boolean contains = text.contains("Delfor");
	...
	@Then("Rose&Macdonald Validate Print ASN and Download ASN buttons on the Shipments Details Page.")
	public void interplex_validate_print_asn_and_download_asn_buttons_on_the_shipments_details_page() throws InterruptedException {
		sp = new ShipmentListPOM();
