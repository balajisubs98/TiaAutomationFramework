...
public class PurchaseOrderSD_ButterShoe extends BaseUtility {
	...
	@Given("butter_shoe Launch Tia application Dashboard On the google chrome Browser {string} {string} {string}")
	...
	@Then("butter_shoe Validate Purchase Order Label, MenuButton, Account name with Logo and Notification Bell Icon are available on Purchase Order list page.")
	...
	public void validate_from_date_filter_and_to_date_filter_on_purchase_order_list_page()
		...
		fwait(po.getFdatefilter());
		ac.sendKeys(po.getFdatefilter(), "01/12/2021").perform();
		ac.sendKeys(po.getTodatefilter(), "31/12/2021").perform();
		Thread.sleep(1500);
...
//		r.keyRelease(KeyEvent.VK_ENTER);
		...
		for (int i = 1; i <= webtablerowcount; i++) {
			...
			String webTable = webTable(i, 8, po.getPotable(), false);
			System.out.println(webTable);
			if (webTable.contains("Dec") && webTable.contains("2021")) {
				sa.assertEquals(true, true,"Date Filter Validation is failed");

			} else {
				boolean displayed = po.getNodatavailableBS().isDisplayed();
				sa.assertEquals(displayed, true,"Date Filter Validation is failed");
				if (displayed) {
	...
	@Then("butter_shoe Validate Status Filter on Purchase Order list page.")
			...
			for (int i = 0; i < li.size(); i++) {
				...
				if (webtablerowcount != 0) {
					for (int j = 2; j < webtablerowcount; j++) {
						String webTable = webTable(j, 10, po.getPotable(), false);
	...
	@Then("butter_shoe Validate PartnerOrg Filter on Purchase Order list page.")
	public void validate_partner_org_filter_on_purchase_order_list_page()
		...
		for (int i = 0; i < li.size(); i++) {
			...
			if (webtablerowcount != 0) {
				for (int j = 2; j < webtablerowcount; j++) {
					String webTable = webTable(j, 5, po.getPotable(), false);
					System.out.println(i);
					System.out.println(webTable);
	...
	public void validate_the_select_action_on_purchase_order_list_page()
		...
		try {
			...
			po.getFdatefilter().sendKeys("21-02-2022");
	...
	@Then("butter_shoe Validate Shows Entries on Purchase Order list page.")
	public void validate_shows_entries_on_purchase_order_list_page() throws InterruptedException, IOException {
		...
		Integer rc = webtablerowcount(po.getPotable());
	...
	@Then("butter_shoe Purchase order listing page Failures")
	...
	@Given("butter_shoe Launch Tia application On the chrome Browser {string} {string} {string}")
	...
	@Then("butter_shoe Validate Ship To Label and Logo, Bill To Label and Logo, and PO status are available on the Purchase order details page.")
	...
	@Then("butter_shoe Validate Plus button on the purchase order details page.")
	...
	@Then("butter_shoe Validate Search module on the purchase order details page.")
...
//		ac.keyUp(Keys.ENTER).perform();
		po.getTextboxsrch().sendKeys("745042112013");
	...
	@Then("butter_shoe Validate product details table on the purchase order details page.")
	...
	@Then("butter_shoe purchase order details page Failures")
