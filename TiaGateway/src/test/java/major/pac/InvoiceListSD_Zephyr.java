...
public class InvoiceListSD_Zephyr extends BaseUtility {
	...
	@Given("Zephyr Launch Tia application Dashboard by google chrome Browser {string} {string} {string}")
	...
	@Then("Zephyr Validate Purchase Order Label, MenuButton, Account name with Logo and Notification Bell Icon are available on Invoices listing page.")
	...
	@Then("Zephyr Validate Status Filter on Invoices listing page.")
	...
	li.add("Preparing Shipment");
		...
		for (int i = 0; i < li.size(); i++) {
			...
			il.getStatusFiltersearchzep().sendKeys(li.get(i));
			Actions ac = new Actions(driver);
			ac.keyDown(Keys.ENTER).perform();
			ac.keyUp(Keys.ENTER).perform();
			Thread.sleep(5000);
	...
	li.add("BROSEKG");
		...
		for (int i = 0; i < li.size(); i++) {
			...
			il.getReceiverFiltersearchzep().sendKeys(li.get(i));
			Actions ac = new Actions(driver);
			ac.keyDown(Keys.ENTER).perform();
			ac.keyUp(Keys.ENTER).perform();
			Thread.sleep(5000);
	...
	public void validate_partner_filter_on_invoices_listing_page() throws AWTException, InterruptedException, IOException {
		...
		for (int i = 0; i < li.size(); i++) {
			...
			il.getPartnerFiltersearchzep().sendKeys(li.get(i));
			Actions ac = new Actions(driver);
			ac.keyDown(Keys.ENTER).perform();
			ac.keyUp(Keys.ENTER).perform();
			Thread.sleep(5000);
	...
	public void validate_the_select_action_on_invoices_listing_page() throws InterruptedException {
		...
		if (options.size()>1) {
			...
			scenario.log("There is no options present in Select Action Dropdown");
			sa.fail("There is no options present in Select Action Dropdown");
		}
	...
	@Then("Zephyr Validate Shows Entries on Invoices listing page.")
	public void validate_shows_entries_on_invoices_listing_page() throws InterruptedException, IOException {
		...
		Integer webtablerowcount = webtablerowcount(po.getPotable());
		String text = il.getRecordcount().getText();
		...
		String text2 = il.getRecordcount().getText();
		...
		String text3 = il.getRecordcount().getText();
		...
		String text4 = il.getRecordcount().getText();
	...
	@Then("Zephyr Interplex Invoice listing page Failures")
	...
	@Given("Zephyr Launch Tia application Zephyr Dashboard by google chrome Browser {string} {string} {string}")
	...
	@Then("Zephyr Validate Invoice Number, Invoice Date, Delivery Date, TG Transaction ID, Created Date, Invoice status, Allowance Total,and Payment Terms, label are available on Invoice Details Page.")
	...
	@Then("Zephyr Validate Allowance popup window, Print Invoice,Download Invoice, PO Number link and ASN Link")
	...
	@Then("Zephyr Invoice details page Failures")
