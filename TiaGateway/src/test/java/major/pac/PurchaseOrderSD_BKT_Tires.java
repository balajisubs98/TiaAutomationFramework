...
public class PurchaseOrderSD_BKT_Tires extends BaseUtility {
	...
	@Given("BKT_Tires Launch Tia application Dashboard On the google chrome Browser {string} {string} {string}")
	...
	@Then("BKT_Tires Validate Purchase Order Label, MenuButton, Account name with Logo and Notification Bell Icon are available on Purchase Order list page.")
...
//		r.keyRelease(KeyEvent.VK_ENTER);
	...
	@Then("BKT_Tires Validate Status Filter on Purchase Order list page.")
	public void validate_status_filter_on_purchase_order_list_page()
		...
		try {
			...
			for (int i = 0; i < li.size(); i++) {
				...
				Thread.sleep(1800);
				po.getStatusFiltersearchBKT().sendKeys(li.get(i));
	...
	public void validate_receiver_filter_on_purchase_order_list_page()
		...
		for (int i = 0; i < li.size(); i++) {
			...
			po.getRecvFiltersearchBKT().sendKeys(li.get(i));
	...
	@Then("BKT_Tires Validate PartnerOrg Filter on Purchase Order list page.")
	public void validate_partner_org_filter_on_purchase_order_list_page()
		...
		li.add("Aptiv");
		li.add("Bohnenkamp");
		li.add("Dynamic Tire Co");
		li.add("TBC");

		for (int i = 0; i < li.size(); i++) {

			po.getPartorgFilter().click();
			po.getPartorgFiltersearchBKT().sendKeys(li.get(i));
			Actions ac = new Actions(driver);
			ac.keyDown(Keys.ENTER).perform();
			ac.keyUp(Keys.ENTER).perform();
			Thread.sleep(5000);
	...
	public void validate_partner_filter_on_purchase_order_list_page()
		...
		for (int i = 0; i < li.size(); i++) {
			...
			po.getPartnerFiltersearchBKT().sendKeys(li.get(i));
			Actions ac = new Actions(driver);
			ac.keyDown(Keys.ENTER).perform();
			ac.keyUp(Keys.ENTER).perform();
			Thread.sleep(5000);
	...
	@Then("BKT_Tires Validate Region Filter on Purchase Order list page.")
	public void validate_region_filter_on_purchase_order_list_page()
		...
		for (int i = 0; i < li.size(); i++) {
			...
			po.getRegionFiltersearchBKT().sendKeys(li.get(i));
			Actions ac = new Actions(driver);
			ac.keyDown(Keys.ENTER).perform();
			ac.keyUp(Keys.ENTER).perform();
			Thread.sleep(5000);
			...
			if (webtablerowcount != 0) {
				for (int j = 2; j < webtablerowcount; j++) {
					...
					if (equals == true) {
						...
						sa.assertEquals(true, webTable.contains(li.get(i)),"Region Filter Validation is failed");
	...
	@Then("BKT_Tires Validate Shows Entries on Purchase Order list page.")
	...
	@Then("BKT_Tires Purchase order listing page Failures")
	...
	@Given("BKT_Tires Launch Tia application On the chrome Browser {string} {string} {string}")
	...
	@Then("BKT_Tires Validate Ship To Label and Logo, Bill To Label and Logo, and PO status are available on the Purchase order details page.")
	...
	@Then("BKT_Tires Validate Plus button on the purchase order details page.")
	...
	public void validate_download_pdf_print_pdf_and_donwload_csv_buttons_are_in_purchase_order_details_page()
		...
		Thread.sleep(3000);
		String title = driver.getTitle();
		boolean contains = title.contains("PdfIframe");
		if (contains==true) {
			sa.assertEquals(driver.getTitle(), "PdfIframe");
			driver.navigate().back();	
		}else {
			sa.assertEquals(true, false,"Print Pdf Button is not working");
		}
		

	}

	@Then("BKT_Tires Validate Search module on the purchase order details page.")
...
//		ac.keyDown(Keys.ENTER).perform();
//		ac.keyUp(Keys.ENTER).perform();
		po.getTextboxsrch().sendKeys("200");
		...
		for (int a = 1; a < 2; a++) {
			...
			String webTable = webTableinnerText(a, 1, po.getProducttable());
			System.out.println(webTable);
	...
	@Then("BKT_Tires Validate product details table on the purchase order details page.")
	public void validate_product_details_table_on_the_purchase_order_details_page()
		...
		elements.add(po.getEansortBKT());
		elements.add(po.getBuyerpartnumsortBKT());
		elements.add(po.getQuantitysortBKT());
		elements.add(po.getItemcostsortBKT());
		elements.add(po.getPricesortBKT());
		elements.add(po.getUnackqtysortBKT());
		...
		for (int k = 1; k <= proacrowcount; k++) {
			String webtableheadinnerText = webTableinnerText(k, 1, po.getProducttable());
			proclow.add(webtableheadinnerText);
		}
		boolean Resultproalow = true;
		System.out.println(proclow);
		System.out.println(procac);
		for (int l = 0; l < proclow.size(); l++) {
			boolean equals = proclow.get(l).equals(procac.get(l));
			System.out.println(equals);
			if (equals == false) {
				Resultproalow = false;
		...
		//po.getLastsortbkt().click();
		Thread.sleep(2000);
		takeScreenshot(screenshotpath, "EanCode_HightoLow_Sort");
		Integer prodcrowcount = webtablerowcount(po.getProducttable());
		List<String> prochigh = new LinkedList<>();
		for (int y = 1; y <= prodcrowcount; y++) {
			String webtableheadinnerText = webTableinnerText(y, 1, po.getProducttable());
			prochigh.add(webtableheadinnerText);
		}
		boolean Resultproahigh = true;
		for (int z = 0; z < prochigh.size(); z++) {
			boolean equals = prochigh.get(z).equals(procdc.get(z));
			System.out.println(equals);
			if (equals == false) {
		...
		po.getDescriptionsortBKT().click();
		...
		po.getEansortBKT().click();
		
		boolean Resultdes2 = false;
		sa.assertEquals(Resultdes2, true, "Eansort low to high sort is not working");
		sa.assertEquals(Resultdes2, true, "Eansort High to low sort is not working");
		
		
		po.getBuyerpartnumsortBKT().click();
		
		boolean Resultdes3 = false;
		sa.assertEquals(Resultdes3, true, "BuyerPartNum low to high sort is not working");
		sa.assertEquals(Resultdes3, true, "BuyerPartNum High to low sort is not working");
		po.getBuyerpartnumsortBKT().click();
		
		po.getReshipdatesortBKT().click();
		boolean Resultdes1 = true;
		sa.assertEquals(Resultdes1, true, "Shipdate sort low to high sort is not working");
		sa.assertEquals(Resultdes1, true, "Shipdate sort High to low sort is not working");
	...
	@Then("BKT_Tires purchase order details page Failures")
