...
public class InvoiceListSD_ButterShoe extends BaseUtility {
	...
	@Given("butter_shoe Launch Tia application Dashboard by google chrome Browser {string} {string} {string}")
	...
	public void validate_after_clicks_invoices_list_option_it_navigates_to_invoices_listing_page() throws IOException {
		...
		js.executeScript("arguments[0].click()", il.getSendInvoices());
		takeScreenshot(screenshotpath, "SendInvoice_Interplex");
		js.executeScript("arguments[0].click()", il.getInvoiceList());
		fwaitpage("Invoices");
	...
	@Then("butter_shoe Validate Purchase Order Label, MenuButton, Account name with Logo and Notification Bell Icon are available on Invoices listing page.")
	...
	@Then("butter_shoe Validate Status Filter on Invoices listing page.")
...
//		li.add("DPH-H004");
	...
	@Then("butter_shoe Validate PartnerOrg Filter on Invoices listing page.")
...
//		li.add("Aptiv Hugary : Szombathely");
	...
	@Then("butter_shoe Validate Shows Entries on Invoices listing page.")
	...
	@Then("butter_shoe Invoice listing page Failures")
	...
	@Given("butter_shoe Launch Tia application butter_shoe Dashboard by google chrome Browser {string} {string} {string}")
	...
	public void validate_order_to_cash_send_invoices_invoice_list_invoice_details() throws IOException, InterruptedException {
		...
		jsclick(po.getOtoC());
		takeScreenshot(screenshotpath, "OtoC");
		jsclick(il.getSendInvoices());
		takeScreenshot(screenshotpath, "Received_Orders");
		jsclick(il.getInvoiceList());
		Thread.sleep(3000);
		fwait(il.getInvoicesLabel());
		String title = driver.getTitle();
		...
		if (rowcount != 0) {
			...
			webTable(1, 0, po.getPotable(), true);
	...
	@Then("butter_shoe Validate Invoice Number, Invoice Date, Delivery Date, TG Transaction ID, Created Date, Invoice status, Allowance Total,and Payment Terms, label are available on Invoice Details Page.")
	public void validate_invoice_number_invoice_date_delivery_date_tg_transaction_id_created_date_invoice_status_allowance_total_and_payment_terms_label_are_available_on_invoice_details_page() throws InterruptedException {
		...
		boolean displayed4 = il.getInvoiceStatus().isDisplayed();
		sa.assertEquals(displayed4, true,"Invoice status Label Validation");
		boolean displayed5 = il.getAllowanceTotalLbl().isDisplayed();
	...
	@Then("butter_shoe Validate Allowance popup window, Print Invoice and Download Invoice.")
	...
	@Then("butter_shoe Invoice details page Failures")
