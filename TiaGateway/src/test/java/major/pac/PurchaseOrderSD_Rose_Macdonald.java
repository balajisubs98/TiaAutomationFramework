...
public class PurchaseOrderSD_Rose_Macdonald extends BaseUtility {
	...
	@Given("Launch Rose&Macdonald Tia application Dashboard On the google chrome Browser {string} {string} {string}")
	public void launch_tia_application_dashboard_on_the_google_chrome_browser(String string, String Username,
		...
		sa.assertEquals(displayed, true,"Dashborad page is not reached");
	...
	public void validate_after_clicks_order_list_option_it_navigates_to_purchase_order_list_page()
		...
		js.executeScript("arguments[0].click()", po.getProcuretopay());
		takeScreenshot(screenshotpath, "OtoC");
		js.executeScript("arguments[0].click()", po.getPurchaseorders());
		takeScreenshot(screenshotpath, "Received_Orders");
		js.executeScript("arguments[0].click()", po.getPolist());
		Thread.sleep(5000);
		String title = driver.getTitle();
		takeScreenshot(screenshotpath, "Purchase_Order_Listing_Page");
		sa.assertEquals(title, "Purchase Order","Purchase Order Page is not reached");
		...
		sa.assertEquals(equals, false,"Purchase Order page title is Incorrect");
	}

	@Then("Validate Rose&Macdonald Purchase Order Label, MenuButton, Account name with Logo and Notification Bell Icon are available on Purchase Order list page.")
	public void validate_notification_label_menu_button_account_name_with_logo_and_notification_bell_icon_are_available_on_purchase_order_list_page() {
		po = new PurchaseOrderPOM();
		fwait(po.getPurchaseOrderLblamar());
		boolean displayed = po.getPurchaseOrderLblamar().isDisplayed();
		sa.assertEquals(displayed, true,"Purchase order Label is not shown");
		if (displayed == true) {
		...
		sa.assertEquals(displayed2, true,"Menu Button is not shown");
		if (displayed2 == true) {
		...
		sa.assertEquals(displayed3, true,"Account Link Logo is not shown");
		if (displayed3 == true) {
		...
		sa.assertEquals(displayed4, true,"Notification Bell Logo is not shown");
		if (displayed4 == true) {
	...
	public void validate_from_date_filter_and_to_date_filter_on_purchase_order_list_page()
		...
		for (int i = 1; i <= webtablerowcount; i++) {
			...
			if (webTable.contains("Jul") && webTable.contains("2023")) {
				sa.assertEquals(true, true,"Date Filter Validation");

			} else {
				sa.assertEquals(true, false,"Date Filter Validation");
	...
	@Then("Validate Rose&Macdonald Status Filter on Purchase Order list page.")
	public void validate_status_filter_on_purchase_order_list_page()
		...
		try {
			...
			for (int i = 0; i < li.size(); i++) {
				...
				Thread.sleep(1800);
				po.getStatusFiltersearch().sendKeys(li.get(i));
				...
				if (webtablerowcount != 0) {
					for (int j = 2; j < webtablerowcount; j++) {
						...
						if (equals == true) {
							...
							sa.assertEquals(true, webTable.contains(li.get(i)),"Status Filter Validation");
	...
	public void validate_receiver_filter_on_purchase_order_list_page()
		...
		for (int i = 0; i < li.size(); i++) {
			...
			po.getRecvFiltersearch().sendKeys(li.get(i));
			...
			if (webtablerowcount != 0) {
				for (int j = 2; j < webtablerowcount; j++) {
					...
					if (equals == true) {
						...
						sa.assertEquals(true, webTable.contains(li.get(i)),"Receiver Filter Validation");
	...
	@Then("Validate Rose&Macdonald PartnerOrg Filter on Purchase Order list page.")
	public void validate_partner_org_filter_on_purchase_order_list_page()
		...
		for (int i = 0; i < li.size(); i++) {
			...
			po.getPartorgFiltersearch().sendKeys(li.get(i));
			Robot r = new Robot();
			r.keyPress(KeyEvent.VK_ENTER);
			r.keyRelease(KeyEvent.VK_ENTER);
			Thread.sleep(5000);
			...
			if (webtablerowcount != 0) {
				for (int j = 2; j < webtablerowcount; j++) {
					...
					if (equals == true) {
						...
						sa.assertEquals(true, webTable.contains(li.get(i)),"Partner Org Filter Validation");
	...
	public void validate_partner_filter_on_purchase_order_list_page()
		...
		for (int i = 0; i < li.size(); i++) {
			...
			po.getPartnerFiltersearch().sendKeys(li.get(i));
			Robot r = new Robot();
			r.keyPress(KeyEvent.VK_ENTER);
			r.keyRelease(KeyEvent.VK_ENTER);
			Thread.sleep(5000);
			...
			if (webtablerowcount != 0) {
				for (int j = 2; j < webtablerowcount; j++) {
					...
					if (equals == true) {
						...
						sa.assertEquals(true, webTable.contains(li.get(i)),"Partner Filter Validation");
	...
	@Then("Validate Rose&Macdonald Region Filter on Purchase Order list page.")
	public void validate_region_filter_on_purchase_order_list_page()
			throws AWTException, InterruptedException, IOException {
		po = new PurchaseOrderPOM();
		JavascriptExecutor js = (JavascriptExecutor) driver;
		List<String> li = new LinkedList<>();
		li.add("ASIA");
		li.add("EUROPE");
		li.add("North America");
		li.add("US");

		for (int i = 0; i < li.size(); i++) {

			po.getRegionFilter().click();
			po.getRegionFiltersearch().sendKeys(li.get(i));
			Robot r = new Robot();
			r.keyPress(KeyEvent.VK_ENTER);
			r.keyRelease(KeyEvent.VK_ENTER);
			Thread.sleep(5000);
			takeScreenshot(screenshotpath, "Purchase_Order_region_Filter");
			Integer webtablerowcount = webtablerowcount(po.getPotable());
			System.out.println("rowCount" + webtablerowcount);
			if (webtablerowcount != 0) {
				for (int j = 2; j < webtablerowcount; j++) {
					String webTable = webTable(j, 3, po.getPotable(), false);
					System.out.println(i);
					System.out.println(webTable);
					boolean equals = webTable.equals(li.get(i));
					if (equals == true) {

						sa.assertEquals(true, webTable.contains(li.get(i)),"Region Filter Validation");
	...
	public void validate_serach_text_box_on_purchase_order_list_page()
		...
		po = new PurchaseOrderPOM();
		JavascriptExecutor js = (JavascriptExecutor) driver;
		String ValidData = "POYP-2324-01178";
		...
		sa.assertEquals(result, true,"Search with Valid data, It not shows proper search result");
		Thread.sleep(2000);
		js.executeScript("arguments[0].click()", po.getResetbtn());
		...
		try {
			...
			sa.assertEquals(displayed2, true, "No Data Available");

		} catch (Exception e) {
			sa.assertTrue(true, "No Data Available");
		}
	...
	public void validate_the_select_action_on_purchase_order_list_page()
		...
		try {
			if (result) {
				...
				sa.assertEquals(textt, "Please select purchase order to create PDF");
				...
				sa.assertEquals(texttt, "Please select purchase order to create CSV");
			...
			if (result1==true) {
				...
				WebElement toastmessage2 = driver.findElement(By.cssSelector("#feedbackMessageContainer > div > div"));
				String textt = toastmessage2.getText();
				sa.assertEquals(textt, "File Downloading..");
				String text3 = options.get(2).getText();
				s.selectByVisibleText("Download CSV");
	...
	@Then("Validate Rose&Macdonald Shows Entries on Purchase Order list page.")
	public void validate_shows_entries_on_purchase_order_list_page() throws InterruptedException, IOException {
		...
		sa.assertEquals(equals, true, "ShowEntries 10");
		Thread.sleep(2000);
		...
		sa.assertEquals(equals2, true, "ShowEntries 25");
		Thread.sleep(2000);
		...
		sa.assertEquals(equals3, true, "ShowEntries 50");
		Thread.sleep(2000);
		...
		boolean equals4 = webtablerowcount4.equals(61);
		System.out.println(webtablerowcount4 + "count(100)");
		System.out.println(equals4);
		sa.assertEquals(equals4, true, "ShowEntries 100");
		Thread.sleep(2000);
		js.executeScript("arguments[0].click()", po.getResetbtn());
		Thread.sleep(4000);
	...
	public void validate_sort_functionality_of_po_number_po_date_shipment_date_and_stauts_on_purchase_order_list_page()
		...
		webele.add(po.getPosortAR());
		webele.add(po.getPodatesortAR());
		webele.add(po.getShipdatesortAR());
		webele.add(po.getStatussortAR());
		...
		boolean equals = Initialsorted.equals(finalsorted);
			...
			sa.assertEquals(equals, false, "sort functionality");
	...
	@Given("Launch Rose&Macdonald Tia application On the chrome Browser {string} {string} {string}")
	...
	public void validate_order_to_cash_receive_orders_order_list_select_a_po_from_list_click_button_should_be_reach_purchase_order_details_page()
		...
		jsclick(po.getProcuretopay());
		takeScreenshot(screenshotpath, "PtoP");
		jsclick(po.getPurchaseorders());
		takeScreenshot(screenshotpath, "Received_Orders");
		jsclick(po.getPolist());
		Thread.sleep(5000);

		String title1 = driver.getTitle();
		takeScreenshot(screenshotpath, "Purchase_Order_Listing_Page");
		sa.assertEquals(title1, "Purchase Order");
		fwait(po.getPotable());
		Thread.sleep(3000);
		...
		Thread.sleep(5000);
		if (rowcount != 0) {
			String liReceiver = webTable(2, 2, po.getPotable(), false);
			tabledetails.add(liReceiver);
			String liRegion = webTable(2, 3, po.getPotable(), false);
			tabledetails.add(liRegion);
			String liPartner = webTable(2, 4, po.getPotable(), false);
			tabledetails.add(liPartner);
			String liPoNum = webTable(2, 5, po.getPotable(), false);
			tabledetails.add(liPoNum);
			String liPODate = webTable(2, 6, po.getPotable(), false);
			tabledetails.add(liPODate);
			String liPOShipDate = webTable(2, 7, po.getPotable(), false);
			tabledetails.add(liPOShipDate);
			String liPoStatus = webTable(2, 9, po.getPotable(), false);
	...
	public void validate_puchase_order_details_po_number_tg_transaction_id_order_date_delivery_date_so_number_total_order_amount_and_total_order_quantity_labels_are_available_on_purchase_order_details_page() {
		...
		boolean equals = text.equals("PO Number :");
		if (equals == true) {
	...
	@Then("Validate Rose&Macdonald Ship To Label and Logo, Bill To Label and Logo, and PO status are available on the Purchase order details page.")
	public void validate_ship_to_label_and_logo_bill_to_label_and_logo_and_po_status_are_available_on_the_purchase_order_details_page() {
		...
		sa.assertEquals(displayed1, true);
		if (displayed1) {
		...
		sa.assertEquals(displayed4, true, "\"po status label\"");
		...
		sa.assertEquals(contains, true, "po status label");
		if (contains) {
	...
	public void validate_history_module_on_the_purchase_order_details_page() throws InterruptedException {
		...
		if (displayed) {
			sa.assertEquals(displayed, true, "purchase order history label");
		}
		Thread.sleep(3000);
		...
		sa.assertEquals(displayed2, true, "po history Tia Gateway Label");
		if (displayed2) {
		...
		sa.assertEquals(displayed3, true, "po history Tangentia link");
		if (displayed3) {
	...
	@Then("Validate Rose&Macdonald Plus button on the purchase order details page.")
	...
	@Then("Validate Rose&Macdonald Search module on the purchase order details page.")
	public void validate_search_module_on_the_purchase_order_details_page() throws InterruptedException, AWTException {
		...
		po.getTextboxsrch().sendKeys("745042400028");
...
//		ac.keyUp(Keys.ENTER).perform();
	...
	@Then("Validate Rose&Macdonald product details table on the purchase order details page.")
